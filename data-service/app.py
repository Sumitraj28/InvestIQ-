...
def normalize_ticker(ticker: str) -> str:
    ...
    if not clean or "." not in clean:
        raise HTTPException(
            status_code=400,
            detail="Ticker must include an exchange suffix, for example RELIANCE.NS.",
        )
    return clean
...
def health():
    return {
        "status": "ok",
        "service": "StockSense Data Service",
        ...
        predictions.append(
            {
                ...
                "predictedClose": round(max(float(predicted_close), 0), 2),
            }
        )
